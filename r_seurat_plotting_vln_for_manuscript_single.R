...
system(paste("rm -rf ", dir.out, "; mkdir ",dir.out,sep=""))
        ...
        p <- VlnPlot(object = so, point.size.use = 0, features.plot = c(gene.target),
                size.x.use = 0,
                size.y.use = 1,
                size.title.use = 7,
...
#--------------------------------------
	for( i in 1:length(genes.in.the.list) ) {
		...
		gene <- genes.in.the.list[[i]]
        	...
        	pdf(file=file.img, bg='transparent', width=4, height=3); print(p); dev.off()

        	file.img <- paste(dir.out,"/","individual_vln_",gene,".png",sep="")
        	png(file=file.img, bg='transparent', width=400, height=300); print(p); dev.off()
	}
